...
def make_mesh(expfile, meshfile, engine,
    ...
    # Refine where the surface slope is steep
    feats = np.load('../../../../analysis/mean/features_G-H.pkl', allow_pickle=True)
    feats_mesh = np.load('../../../G-H/data/geom/mesh.npy', allow_pickle=True)
    feats_levelset = np.load('../../../G-H/data/geom/ocean_levelset.npy')
    feats_xy = (feats_mesh['x'][feats_levelset>0], feats_mesh['y'][feats_levelset>0])
    feats_slope = feats['potential_slope']
    feats_slope[np.isnan(feats_slope)] = 0
    shreve_slope = griddata(feats_xy, feats_slope, (md.mesh.x, md.mesh.y))
    slope_threshold = 200
    area[shreve_slope>=slope_threshold] = hmin
    ...
    mesh_ocean_levelset[mesh_bm_mask<0.5] = -1 # Bedmachine ocean
