...
fig,ax = plt.subplots()
pc = ax.tripcolor(mtri, coef, vmin=0, vmax=np.quantile(coef, 0.99))
...
# fig,ax = plt.subplots()
# ax.tripcolor(mtri, model_vel)
# ax.set_title('Model vel')
# fig.savefig('model_vel.png', dpi=400)

# obs_vel = np.load('obs_vel.npy').squeeze()
vx = np.load('../data/geom/vx.npy')
vy = np.load('../data/geom/vy.npy')
obs_vel = np.sqrt(vx**2 + vy**2)
print('obs_vel.shape:', obs_vel.shape)
# fig,ax = plt.subplots()
# ax.tripcolor(mtri, obs_vel)
# ax.set_title('Obs vel')
# fig.savefig('obs_vel.png', dpi=400)

model_vel[levelset<1] = np.nan
obs_vel[levelset<1] = np.nan

fig, axs = plt.subplots(figsize=(8, 8), ncols=2, nrows=2)
axs = axs.flatten()

abs_pc = axs[0].tripcolor(mtri, np.log10(obs_vel + 1e-1), vmin=0, vmax=3)
