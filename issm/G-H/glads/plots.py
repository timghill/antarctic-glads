...
mesh = np.load('../data/geom/amery_mesh.npy', allow_pickle=True)
mtri = Triangulation(mesh['x'], mesh['y'], mesh['elements']-1)

# RUNTIME
time = np.load('train_runtime.npy').squeeze()
...
fig.savefig('ensemble_runtime.png', dpi=400)
plt.close(fig)


# EFFECTIVE PRESSURE
N = np.load('train_N.npy')[:, -1, :]
print(N.shape)
N_ensmean = np.nanmean(N, axis=-1)

Q = np.abs(np.load('train_Q.npy')[:,-1,:])
Q_tot = np.sum(Q, axis=0)

N_mean = np.load('train_N_mean.npy')[-1]
fw_mean = np.load('train_ff_mean.npy')[-1]
...
ax.set_xlabel('N (KPa)')
ax.set_ylabel('Count')
fig.savefig('amery_train_ensemble_hist_N.png', dpi=400)
plt.close(fig)
...
ax.set_ylabel('Count')
fig.savefig('amery_train_ensemble_hist_ff.png', dpi=400)
plt.close(fig)
...
fig.savefig('amery_train_ensemble_map_N.png', dpi=400)
plt.close(fig)

# fig,ax = plt.subplots()
fig = plot_map(mtri, fw_ensmean, vmin=0, vmax=1, 
    cmap=cmocean.cm.dense, label='Flotation fraction')
fig.savefig('amery_train_ensemble_map_ff.png', dpi=400)
plt.close(fig)
...
fig.savefig('amery_train_ensemble_map_quantiles.png', dpi=800)
...
fig.savefig('amery_train_ensemble_runtime.png', dpi=400)
...
fig.savefig('amery_train_ensemble_N.png', dpi=400)
plt.close(fig)
# plt.show()

fig, axs = plt.subplots(nrows=3, ncols=2, figsize=(6, 6), sharex=False, sharey=True)
Q = np.abs(np.load('train_Q.npy')[:, :, :])
...
fig.savefig('amery_train_ensemble_Q.png', dpi=400)
plt.close(fig)

fig,ax = plt.subplots()
tt = np.linspace(0, 10, 101)
ax.plot(tt, Qsum)
fig.savefig('amery_train_ensemble_Q_timeseries.png', dpi=400)
# # plt.show()
