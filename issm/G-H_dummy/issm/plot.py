...
axs[0].tripcolor(mtri, np.load('solutions/u_glads_present.npy'), vmin=0, vmax=2.5e3)
axs[0].set_title('GlaDS')
pc = axs[1].tripcolor(mtri, np.load('solutions/u_rf_future.npy'), vmin=0, vmax=2.5e3)
axs[1].set_title('RF')
fig.colorbar(pc, ax=axs, label='Speed (m/year)')
...
interp_speed = griddata((mesh['x'], mesh['y']), u, (xx, yy), method='linear')
