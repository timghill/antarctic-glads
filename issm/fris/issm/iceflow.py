...
def set_para(effective_pressure, initialization=None):
    ...
    md.inversion.incomplete_adjoint = 1 # 0
...
def run_friction_inversion(effective_pressure, initialization=None,
    ...
    md.inversion.cost_functions_coefficients[md.inversion.vel_obs<1, 0:2] = 0
...
if __name__=='__main__':
    ...
    g = 9.81
    rhowater = 1028
    pice = rhoice*g*np.load('../data/geom/thick.npy')
    bed = np.load('../data/geom/bed.npy')
    pwater = np.maximum(0, -rhowater*g*bed)
    N = pice - pwater
    levelset = np.load('../data/geom/ocean_levelset.npy')
    N[levelset<0] = 0
